...
class ApiManager {
    ...
    companion object {
        ...
        const val URL_GET_DEVICE:String = "$URL_HTTP$URL_SERVER_DOMAIN/others/device/"
    ...
    fun getDeviceParams() {
        checkNetwork(object:NetworkListener{
            override fun valid() {
                AndroidNetworking.get(URL_GET_DEVICE)
                        .addHeaders("API-AUTH", "a69caa73-f126-444d-879c-74e75d433940")
                        .build()
                        .getAsOkHttpResponse(object:OkHttpResponseListener{
                            override fun onResponse(response: Response?) {
                                responseSucceed(response,"设备信息请求成功")
                            }

                            override fun onError(anError: ANError?) {
                                responseError(anError,"设备信息请求失败")
                            }
                        })
            }

            override fun unvalid() {
                responseError("请求设备信息", "网络连接失败")
            }
        })
    }
