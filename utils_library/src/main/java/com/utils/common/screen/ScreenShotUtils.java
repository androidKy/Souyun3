...
public class ScreenShotUtils {
    ...
    public ScreenShotUtils(Context context, int resultCode, Intent intent) {
        this.mRefContext = new SoftReference<>(context);

        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {

            mMediaProjection = getMediaProjectionManager().getMediaProjection(resultCode, intent);
    ...
    public void startScreenShot(OnShotListener onShotListener, String picUrl) {
        mLocalUrl = picUrl;
    ...
    public void startScreenShot(OnShotListener onShotListener) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            ...
            if (getContext() != null) {
                ToastUtils.Companion.showToast(getContext(), "Android系统版本过低，暂不支持截屏");
            }
        }
    ...
    public class SaveTask extends AsyncTask<Image, Void, Bitmap> {
        ...
        protected Bitmap doInBackground(Image... params) {
            ...
            bitmap = Bitmap.createBitmap(bitmap, 350, 240, 400, 150);
            image.close();
            File fileImage = null;
            if (bitmap != null) {
                try {
                    ...
                    if (TextUtils.isEmpty(mLocalUrl)) {
                        mLocalUrl = getContext().getExternalFilesDir("screenshot").getAbsoluteFile().getPath()
                                +
                                "/"
                                +
